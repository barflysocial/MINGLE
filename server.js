...
function defaultEvent() {
  return {
    ...
    roundStartsAt: new Date(Date.now() + 10 * 60 * 1000).toISOString(),
...
function ensureRoundSettings(db) {
  ...
  db.event.roundStartsAt ||= new Date(Date.now() + 10 * 60 * 1000).toISOString();
  if (typeof db.event.autoStartEnabled !== 'boolean') db.event.autoStartEnabled = true;
  db.event.roundStatus ||= db.event.roundStartedAt ? 'started' : 'scheduled';
  return db.event;
}
