...
const QUESTIONS = {
  ...
  dealbreakers: {
    ...
    options: ['Drama', 'Inconsistency', 'Heavy partying', 'Smoking', 'Drugs', 'Poor communication', 'Rushing intimacy', 'No ambition', 'Does not want commitment', 'Dishonesty']
  },
  greenflags: {
    field: 'greenflags',
    multi: true,
...
function ensureVenues(db) {
  ...
  db.venues.forEach(v => {
    ...
    v.address ||= '';
    v.city ||= '';
    if (typeof v.isPaid !== 'boolean') v.isPaid = false;
    v.paymentLink ||= '';
    v.drinkSpecials ||= '';
    if (!('seatCap' in v) || v.seatCap === '' || v.seatCap === null) v.seatCap = 50;
    v.seatCap = Math.max(0, Number(v.seatCap) || 0);
    v.createdAt ||= now();
  });
  return db.venues;
}
...
function publicVenue(venue) {
  return {
    ...
    drinkSpecials: venue.drinkSpecials || '',
    seatCap: Number(venue.seatCap) || 0,
    status: venue.status || 'open'
  };
}

function checkedInCountForVenue(db, venueId) {
  return db.players.filter(p => p.checkedIn && p.checkedInVenueId === venueId).length;
}

function hostVenue(db, venue) {
  const count = checkedInCountForVenue(db, venue.id);
  const cap = Number(venue.seatCap) || 0;
  return {
    ...publicVenue(venue),
    token: venue.token,
    checkedInCount: count,
    seatsRemaining: cap ? Math.max(0, cap - count) : null,
    isFull: cap ? count >= cap : false
  };
}

function interestHotSpots(db) {
  const checked = db.players.filter(p => p.checkedIn && p.ready);
  const countBy = (getter) => {
    const out = {};
    checked.forEach(p => {
      const raw = getter(p) || 'Not answered';
      const values = Array.isArray(raw) ? raw : [raw];
      values.forEach(v => {
        const key = normalize(v) || 'Not answered';
        out[key] = (out[key] || 0) + 1;
      });
    });
    return Object.entries(out).map(([label, count]) => ({ label, count })).sort((a, b) => b.count - a.count || a.label.localeCompare(b.label));
  };
  return {
    totalCheckedInReady: checked.length,
    intent: countBy(p => p.profile?.intent),
    matchScope: countBy(p => p.profile?.matchScope),
    ageRange: countBy(p => p.profile?.ageRange),
    values: countBy(p => p.profile?.values)
  };
}


function publicPreview(db) {
  ensureVenues(db);
  const venues = db.venues.map(v => {
    const hv = hostVenue(db, v);
    return {
      id: hv.id,
      name: hv.name,
      address: hv.address,
      city: hv.city,
      eventTitle: hv.eventTitle,
      eventTime: hv.eventTime,
      logoUrl: hv.logoUrl,
      isPaid: hv.isPaid,
      paymentLink: hv.paymentLink,
      drinkSpecials: hv.drinkSpecials,
      seatCap: hv.seatCap,
      checkedInCount: hv.checkedInCount,
      seatsRemaining: hv.seatsRemaining,
      isFull: hv.isFull,
      status: hv.status
    };
  });
...
function absoluteUrl(req, pathValue) {
  ...
  const cleanPath = String(pathValue || '/').startsWith('/') ? pathValue : `/${pathValue}`;
  if (WEB_BASE_URL) return `${WEB_BASE_URL}${cleanPath}`;
  const proto = req.headers['x-forwarded-proto'] || 'https';
  const host = req.headers['x-forwarded-host'] || req.headers.host || 'localhost';
  return `${proto}://${host}${cleanPath}`;
}

function injectRichLinkMeta(req, html) {
  const pageUrl = absoluteUrl(req, '/');
  const imageUrl = absoluteUrl(req, '/one-match-ai-preview.png');
  return html
    .replace(/<meta property="og:url" content="[^"]*" \/>/g, '')
    .replace(/<meta property="og:image" content="[^"]*" \/>/g, `<meta property="og:image" content="${imageUrl}" />`)
    .replace(/<meta property="og:image:secure_url" content="[^"]*" \/>/g, `<meta property="og:image:secure_url" content="${imageUrl}" />`)
    .replace(/<meta name="twitter:image" content="[^"]*" \/>/g, `<meta name="twitter:image" content="${imageUrl}" />`)
    .replace(/<link rel="canonical" href="[^"]*" \/>/g, `<link rel="canonical" href="${pageUrl}" />`)
    .replace(/<link rel="image_src" href="[^"]*" \/>/g, `<link rel="image_src" href="${imageUrl}" />`)
...
function serveStatic(req, res) {
  ...
  if (requested === '/demo') requested = '/demo.html';
  if (requested === '/demo-host') requested = '/demo-host.html';
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    if (ext === '.html' && path.basename(filePath) === 'index.html') {
      data = Buffer.from(injectRichLinkMeta(req, data.toString('utf8')), 'utf8');
    }
    const types = {
...
async function handleApi(req, res) {
  ...
  if (method === 'GET' && url.pathname === '/api/public-preview') {
    return send(res, 200, publicPreview(db));
  }
  ...
  if (method === 'POST' && url.pathname.match(/^\/api\/player\/[^/]+\/checkin$/)) {
    ...
    const seatCap = Number(venue.seatCap) || 0;
    const checkedCount = checkedInCountForVenue(db, venue.id);
    if (!player.checkedIn && seatCap && checkedCount >= seatCap) {
      return send(res, 403, { error: `This event is full. Seat cap is ${seatCap}.` });
    }
  ...
  if (method === 'GET' && url.pathname === '/api/host/state') {
    return send(res, 200, {
      event: publicEvent(db),
      venues: db.venues.map(v => hostVenue(db, v)),
      hotSpots: interestHotSpots(db),
  ...
  if (method === 'POST' && url.pathname === '/api/host/venue') {
    ...
    const venue = {
      ...
      name,
      address: normalize(body.address || '').slice(0, 180),
      ...
      logoUrl: normalize(body.logoUrl || '').slice(0, 500),
      isPaid: !!body.isPaid,
      paymentLink: normalize(body.paymentLink || '').slice(0, 500),
      drinkSpecials: normalize(body.drinkSpecials || '').slice(0, 300),
      seatCap: Math.max(0, Number(body.seatCap) || 0),
    ...
    writeDb(db);
    return send(res, 200, { venue: hostVenue(db, venue) });
  }

  if (method === 'POST' && url.pathname.match(/^\/api\/host\/venue\/[^/]+\/update$/)) {
    ...
    if ('name' in body) venue.name = normalize(body.name || venue.name).slice(0, 120);
    if ('address' in body) venue.address = normalize(body.address || '').slice(0, 180);
    if ('city' in body) venue.city = normalize(body.city || '').slice(0, 80);
    if ('eventTitle' in body) venue.eventTitle = normalize(body.eventTitle || 'Mingle Night').slice(0, 120);
    if ('eventTime' in body) venue.eventTime = normalize(body.eventTime || 'Tonight').slice(0, 80);
    if ('logoUrl' in body) venue.logoUrl = normalize(body.logoUrl || '').slice(0, 500);
    if ('isPaid' in body) venue.isPaid = !!body.isPaid;
    if ('paymentLink' in body) venue.paymentLink = normalize(body.paymentLink || '').slice(0, 500);
    if ('drinkSpecials' in body) venue.drinkSpecials = normalize(body.drinkSpecials || '').slice(0, 300);
    if ('seatCap' in body) venue.seatCap = Math.max(0, Number(body.seatCap) || 0);
    if ('status' in body) venue.status = normalize(body.status || venue.status).toLowerCase() === 'closed' ? 'closed' : 'open';
  ...
  if (method === 'POST' && url.pathname.match(/^\/api\/host\/venue\/[^/]+\/delete$/)) {
    const venueId = url.pathname.split('/')[4];
    const body = await parseBody(req);
    if (normalize(body.confirm).toUpperCase() !== 'DELETE') return send(res, 400, { error: 'Type DELETE to confirm event deletion.' });
    ensureVenues(db);
    const venue = db.venues.find(v => v.id === venueId);
    if (!venue) return send(res, 404, { error: 'Venue/event not found.' });

    const affectedPlayerIds = new Set(db.players.filter(p => p.checkedInVenueId === venueId).map(p => p.id));
    db.venues = db.venues.filter(v => v.id !== venueId);

    // Remove venue check-in from affected players and remove matches tied to those checked-in players.
    db.players.forEach(p => {
      if (p.checkedInVenueId === venueId) {
        p.checkedIn = false;
        delete p.checkedInVenueId;
        delete p.checkedInVenueName;
        delete p.checkedInAt;
        delete p.matchId;
        delete p.noMatchMessage;
        p.updatedAt = now();
      }
    });
    db.matches = db.matches.filter(m => !affectedPlayerIds.has(m.playerA) && !affectedPlayerIds.has(m.playerB));

    writeDb(db);
    return send(res, 200, { message: `${venue.name} event deleted.` });
  }
