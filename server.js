...
function findVenueByToken(db, token) {
  ...
  return db.venues.find(v => normalize(v.token) === cleaned || normalize(v.id) === cleaned);
}
...
async function handleApi(req, res) {
  ...
  if (method === 'GET' && url.pathname.startsWith('/api/player/')) {
    ...
    return send(res, 200, { player: publicPlayer(player), question: getQuestionForPlayer(player), match: publicMatch(db, match, player.id), event: db.event, venues: db.venues.map(publicVenue) });
  }
  ...
  if (method === 'POST' && url.pathname === '/api/host/venue') {
    ...
    const venue = {
      ...
      token: `VENUE-${crypto.randomBytes(8).toString('hex').toUpperCase()}`,
      createdAt: now()
    };
