...
function setupStepInfo() {
  ...
  if (!state.player?.liveVerified || stage === 'liveVerify') {
    return { step: 1, total: 4, label: 'Step 1: Take Picture', percent: 15 };
  }
  ...
  if (state.player?.ready && state.player?.avatarNameAssigned) {
    return { step: 4, total: 4, label: 'Step 4: Identity Ready', percent: 100 };
  }
  if (aboutYou.has(stage)) {
    return { step: 2, total: 4, label: 'Step 2: About You', percent: 25 + Math.round((SETUP_STAGES.indexOf(stage) / SETUP_STAGES.length) * 35) };
  }
  if (lookingFor.has(stage)) {
    return { step: 3, total: 4, label: 'Step 3: Who You’re Looking For', percent: 65 + Math.round((SETUP_STAGES.indexOf(stage) / SETUP_STAGES.length) * 25) };
  }
  return { step: 2, total: 4, label: 'Profile Setup', percent: 35 };
}
...
function renderVerification() {
  ...
  const revealIdentity = !!(state.player.ready && state.player.avatarNameAssigned && state.player.avatarImage);
  ...
  if (state.capturedAvatar && $('avatarPreview')) $('avatarPreview').src = state.capturedAvatar;
}

function renderProfileCard() {
  const revealIdentity = !!(state.player.ready && state.player.avatarNameAssigned && state.player.avatarImage);
  $('profileCard').classList.toggle('hide', !revealIdentity);
  if (!revealIdentity) return;
...
function renderQuestion() {
  ...
  $('questionHelper').textContent = q.multi ? `${q.helper || ''} Choose up to ${q.max || 3}.` : (q.helper || '');
  $('choices').innerHTML = '';
  ...
  if (q.liveVerify) {
    $('progressPill').textContent = 'Take Picture';
    $('progressPill').className = 'pill warn';
    $('questionText').textContent = 'Step 1: Take Picture';
    $('questionHelper').textContent = 'Take your live picture to create your pixel avatar.';
    $('choices').innerHTML = '';
    return;
  }

  const stepInfo = setupStepInfo();
  $('progressPill').textContent = q.complete ? 'Profile Ready' : stepInfo.label;
...
function startLobbyCountdown() {
  ...
  const tick = async () => {
    if (!Number.isFinite(startMs)) {
      ...
      $('waitingText').textContent = 'Waiting for the host to schedule the match round.';
      return;
    }
    const left = startMs - serverNowMs();
    if (left > 0) {
      $('lobbyCountdown').textContent = formatCountdown(left);
      $('waitingText').textContent = 'Stay nearby. The match round will start automatically from server time.';
    } else {
      $('lobbyCountdown').textContent = 'Starting…';
      $('waitingText').textContent = 'The server is starting the match round. If nothing changes, the host can start it manually.';
      // Trigger a server-time auto-start check and refresh match status.
      if (!state._pollingLobby) {
...
function renderMatch() {
  ...
  ['topStatusCard'].forEach(id => { if ($(id)) $(id).classList.remove('hide'); });
  stopLobbyCountdown();
