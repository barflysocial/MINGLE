...
function stopPreviewAutoRefresh() {
  if (state.previewRefreshTimer) clearInterval(state.previewRefreshTimer);
...
function startPreviewAutoRefresh() {
  ...
  state.previewRefreshTimer = setInterval(() => {
    if ($('previewPanel') && !$('previewPanel').classList.contains('hide')) loadPublicPreview();
  }, 20000);
}
...
function startPlayerAutoRefresh() {
  ...
  state.playerRefreshTimer = setInterval(() => {
    if (!state.playerId || document.hidden) return;
    const shouldRefresh = !!(state.player?.ready || state.player?.checkedIn || state.match);
    if (shouldRefresh) loadPlayer();
  }, 10000);
}
...
function renderPublicPreview() {
  ...
  if ($('publicHotSpotsList')) {
    $('publicHotSpotsList').innerHTML = [
      ...
      publicHotSpotBlock('Mingle Options Tonight', hot.matchScope)
    ].join('');
  }
}
...
function startLobbyCountdown() {
  ...
  const tick = async () => {
    if (!startsAt || !Number.isFinite(startMs)) {
      ...
      if (!state._lastLobbyPoll || Date.now() - state._lastLobbyPoll > 8000) {
        state._lastLobbyPoll = Date.now();
        loadPlayer();
      }
      return;
    }
...
if ($('previewRefreshBtn')) $('previewRefreshBtn').addEventListener('click', loadPublicPreview);
