...
const QUESTIONS = {
  ...
  values: {
    ...
    helper: 'Choose up to 4. These become the strongest compatibility signals.',
  ...
  dealbreakers: {
    ...
    helper: 'Choose up to 4. Dealbreakers help eliminate poor fits before ranking.',
  ...
  greenflags: {
    ...
    helper: 'Choose up to 4. These help explain why a match is worth your time.',
...
function readDb() {
  ...
  try {
    ...
    return { event: defaultEvent(), players: [], matches: [], reports: [], blocks: [] };
  }
}
...
function getQuestionForPlayer(player) {
  ...
  if (stage === 'complete') {
    return {
      stage: 'complete',
      complete: true,
      question: 'Your One Match AI profile is ready. Check in with the host code, then the AI can narrow the room to your strongest match.',
      helper: 'No endless swiping. One strongest match for this session.',
      options: []
    };
  }
...
function publicMatch(db, match, viewerId) {
  ...
  const revealUnlocked = !!(match.revealRequests?.[match.playerA] && match.revealRequests?.[match.playerB]);
  return {
    id: match.id,
    score: match.score,
    ...
    chat: match.chatUnlocked ? match.chat.map(c => ({
      ...
      avatarName: c.from === 'system' ? 'One Match AI' : (db.players.find(p => p.id === c.from)?.avatarName || 'Player'),
      text: c.text
    })) : [],
...
function hostMatch(db, match) {
  ...
  return {
    id: match.id,
    players: [a?.avatarName || 'Player A', b?.avatarName || 'Player B'],
    score: match.score,
    status: match.status,
    chatUnlocked: !!match.chatUnlocked,
    ...
    prompt: match.prompt,
    reasons: match.reasons,
    revealUnlocked: !!(match.revealRequests?.[match.playerA] && match.revealRequests?.[match.playerB]),
...
async function handleApi(req, res) {
  ...
  if (method === 'GET' && url.pathname === '/api/event') {
    return send(res, 200, { event: db.event });
  }

  if (method === 'POST' && url.pathname === '/api/player/start') {
    const body = await parseBody(req);
    const avatarName = normalize(body.avatarName).slice(0, 32);
    const rsvpCode = normalize(body.rsvpCode).toUpperCase();
    const ageConfirmed = !!body.ageConfirmed;
    if (!ageConfirmed) return send(res, 400, { error: 'You must confirm you are 18+ to join this MVP.' });
    if (!avatarName) return send(res, 400, { error: 'Choose a private avatar name.' });
    if (rsvpCode !== normalize(db.event.rsvpCode).toUpperCase()) return send(res, 403, { error: 'Incorrect RSVP code.' });
    const duplicate = db.players.find(p => lower(p.avatarName) === lower(avatarName));
    if (duplicate) return send(res, 409, { error: 'That avatar name is already taken for this session.' });
    const player = {
      id: id('player'),
      avatarName,
      ageConfirmed: true,
      ...
      stage: 'liveVerify',
      liveVerified: false,
      liveChallenge: pickLiveChallenge(avatarName + now()),
      avatarImage: null,
      livePhoto: null,
      profile: {},
      aiLog: [],
  ...
  if (method === 'POST' && url.pathname.match(/^\/api\/player\/[^/]+\/live-verify$/)) {
    ...
    player.updatedAt = now();
    writeDb(db);
    return send(res, 200, { player: publicPlayer(player), question: getQuestionForPlayer(player), message: 'Live selfie verified. Your cartoon avatar is ready.' });
  }
  ...
  if (method === 'GET' && url.pathname.startsWith('/api/player/')) {
    ...
    return send(res, 200, { player: publicPlayer(player), question: getQuestionForPlayer(player), match: publicMatch(db, match, player.id), event: db.event });
  }
  ...
  if (method === 'POST' && url.pathname.match(/^\/api\/player\/[^/]+\/checkin$/)) {
    ...
    if (!player) return send(res, 404, { error: 'Player not found.' });
    const body = await parseBody(req);
    const code = normalize(body.checkinCode).toUpperCase();
    if (code !== normalize(db.event.checkinCode).toUpperCase()) return send(res, 403, { error: 'Incorrect host check-in code.' });
    if (!player.liveVerified) return send(res, 400, { error: 'Complete live photo verification first.' });
    if (!player.ready) return send(res, 400, { error: 'Finish the AI onboarding questions before check-in.' });
    player.checkedIn = true;
    player.checkedInAt = now();
    player.updatedAt = now();
    writeDb(db);
    return send(res, 200, { player: publicPlayer(player), message: 'Checked in. One Match AI can now include you in the matching funnel.' });
  }
  ...
  if (method === 'POST' && url.pathname.match(/^\/api\/player\/[^/]+\/find-match$/)) {
    ...
    const player = db.players.find(p => p.id === playerId);
    if (!player) return send(res, 404, { error: 'Player not found.' });
    if (!player.liveVerified || !player.checkedIn || !player.ready) return send(res, 400, { error: 'Finish live verification, onboarding, and check in first.' });
    runMatchFunnel(db);
    writeDb(db);
    const match = getMatchForPlayer(db, player.id);
    return send(res, 200, { player: publicPlayer(player), match: publicMatch(db, match, player.id), message: match ? 'One Match AI found your strongest match for this session.' : player.noMatchMessage });
  }
  ...
  if (method === 'GET' && url.pathname === '/api/host/state') {
    return send(res, 200, {
      ...
      reports: db.reports.map(r => ({
        ...r,
        reporter: db.players.find(p => p.id === r.reporterId)?.avatarName || 'Unknown',
        reported: db.players.find(p => p.id === r.reportedId)?.avatarName || 'Unknown'
      })),
  ...
  if (method === 'POST' && url.pathname === '/api/host/full-reset') {
    ...
    const fresh = { event: { ...defaultEvent(), title: db.event.title, venue: db.event.venue, rsvpCode: db.event.rsvpCode, checkinCode: db.event.checkinCode }, players: [], matches: [], reports: [], blocks: [] };
    writeDb(fresh);
