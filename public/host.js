...
function renderVenues() {
  ...
  $('venuesHostList').innerHTML = venues.map(v => `
    <div class="venueHostCard glass" id="venue_${escapeHTML(v.id)}">
      <div class="row space wrap">
        <div class="row wrap" style="align-items:center">
          ...
          <div>
            ...
            ${v.drinkSpecials ? `<p class="small">Drink Special: ${escapeHTML(v.drinkSpecials)}</p>` : ''}
      ...
      <div class="venueManagerSections">
        <div class="venueSection">
          ...
          <button class="secondary saveVenueInfoBtn" data-venue="${escapeHTML(v.id)}">Save Venue</button>
        </div>
        ...
        <div class="venueSection">
          ...
          <div class="grid2 venueEditGrid">
            ...
            <input id="drinks_${escapeHTML(v.id)}" value="${escapeHTML(v.drinkSpecials || '')}" placeholder="Drink specials" />
          ...
          <button class="secondary saveEventDetailsBtn" data-venue="${escapeHTML(v.id)}">Save Event</button>
        </div>
      </div>
  ...
  document.querySelectorAll('.saveVenueInfoBtn').forEach(btn => btn.addEventListener('click', () => saveVenueInfo(btn.dataset.venue)));
  document.querySelectorAll('.saveEventDetailsBtn').forEach(btn => btn.addEventListener('click', () => saveEventDetails(btn.dataset.venue)));
  document.querySelectorAll('.deleteVenueBtn').forEach(btn => btn.addEventListener('click', () => deleteVenue(btn.dataset.venue)));
}

async function saveVenueInfo(venueId) {
  try {
    ...
    msg('venueMsg', 'Venue info saved.', true);
    await loadState();
  } catch (err) { msg('venueMsg', err.message); }
}

async function saveEventDetails(venueId) {
  try {
    await api(`/api/host/venue/${venueId}/update`, { method: 'POST', body: {
      ...
      drinkSpecials: $(`drinks_${venueId}`).value,
    ...
    msg('venueMsg', 'Event details saved. QR code updated.', true);
    await loadState();
  } catch (err) { msg('venueMsg', err.message); }
}
...
$('addVenueBtn').addEventListener('click', async () => {
  try {
    await api('/api/host/venue', { method: 'POST', body: {
      ...
      drinkSpecials: $('newVenueDrinks').value,
