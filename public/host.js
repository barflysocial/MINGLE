...
function renderVenues() {
  ...
  $('venuesHostList').innerHTML = venues.map(v => `
    <div class="venueHostCard glass" id="venue_${escapeHTML(v.id)}">
      ...
      <div class="grid2">
        <input id="token_${escapeHTML(v.id)}" value="${escapeHTML(v.token || '')}" placeholder="Custom token" />
        <button class="secondary saveTokenBtn" data-venue="${escapeHTML(v.id)}">Save Token</button>
      </div>
      <div class="row wrap">
  ...
  document.querySelectorAll('.saveTokenBtn').forEach(btn => btn.addEventListener('click', () => saveVenueToken(btn.dataset.venue)));
}
