...
function readDb() {
  ...
  try {
    ...
    return { event: defaultEvent(), players: [], matches: [], reports: [], blocks: [], usedAvatarNames: [] };
  }
}
...
async function handleApi(req, res) {
  ...
  if (method === 'POST' && url.pathname === '/api/player/start') {
    ...
    const avatarName = generateAvatarName(db);
    const player = {
      id: id('player'),
      avatarName,
      ageConfirmed: true,
  ...
  if (method === 'POST' && url.pathname === '/api/host/full-reset') {
    ...
    const fresh = { event: { ...defaultEvent(), title: db.event.title, venue: db.event.venue, rsvpCode: db.event.rsvpCode, checkinCode: db.event.checkinCode }, players: [], matches: [], reports: [], blocks: [], usedAvatarNames: db.usedAvatarNames || [] };
    writeDb(fresh);
